"""
Domain-level SkillRegistry shared across all strata.

The registry lives in the domain layer (S2) so that infrastructure (S1)
can look up skills without importing from the capability layer (S3).
"""
...
class SkillRegistry:
    ...
    def all_specs(cls) -> List[ToolSpec]:
        """Return only globally valid skills: enabled, not hidden, not dev-only."""
        return [
            spec
