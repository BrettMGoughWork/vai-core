"""stdlib.http.stealth — Stealth browser HTTP GET (Phase 3.11.3).

Uses Playwright with stealth patches, fingerprint masking, and human-like
timing to bypass common anti-bot systems.  ISOLATED from the fallback router
...
# ---------------------------------------------------------------------------
# Viewport / device metric randomisation
# ---------------------------------------------------------------------------
...
_SCREEN_DEVICE_PIXEL: list[dict[str, Any]] = [
    {"screen": {"width": 1920, "height": 1080}, "devicePixelRatio": 1},
    {"screen": {"width": 1920, "height": 1080}, "devicePixelRatio": 2},
    ...
    {"screen": {"width": 1536, "height": 864}, "devicePixelRatio": 1},
    {"screen": {"width": 1680, "height": 1050}, "devicePixelRatio": 1},
]
...
# ---------------------------------------------------------------------------
# Exception classification (mirrors headless_browser for decoupling)
# ---------------------------------------------------------------------------
...
def _human_delay(min_ms: float = 50, max_ms: float = 300) -> None:
    """Sleep for a random human-like delay."""
    time.sleep(random.uniform(min_ms, max_ms) / 1000)


def _build_viewport() -> dict[str, int]:
    """Return a randomised viewport dict."""
    return dict(random.choice(_VIEWPORTS))


def _build_screen_metrics() -> dict[str, Any]:
    """Return a randomised screen-metrics config."""
    return dict(random.choice(_SCREEN_DEVICE_PIXEL))
...
class HttpStealthPrimitive(PrimitiveBase):
    ...
    name = "stdlib.http.stealth"
    description = (
        "Stealth browser HTTP GET with fingerprint masking, "
        "human-like timing, and anti-bot evasion"
    )
    ...
    input_schema = {
        ...
        "properties": {
            "url": {
                "type": "string",
                "description": "URL to load in the stealth browser",
            },
            "timeout": {
                "type": "number",
    ...
    def validate_args(self, args: dict) -> None:
        ...
        if "rate_limit_ms" in args:
            rl = args["rate_limit_ms"]
            if not isinstance(rl, (int, float)):
                raise ValueError(
                    f"'rate_limit_ms' must be a number, got {type(rl).__name__}"
                )
            if rl < 0:
                raise ValueError(f"'rate_limit_ms' must be >= 0, got {rl}")
    ...
    def execute(self, args: dict, context: dict) -> PrimitiveResult:
        ...
        if not _stealth_available:
            return PrimitiveResult(
                ...
                error="playwright-stealth not installed",
            )
        ...
        rate_limit_ms: float = args.get("rate_limit_ms", 300)

        final_headers = _build_headers(user_headers)

        # Human-like delay before navigation
        _human_delay(100, 500)
        ...
        try:
            with sync_playwright() as pw:  # type: ignore[arg-type]
                ...
                try:
                    # Randomised viewport and device metrics
                    viewport = _build_viewport()
                    screen_metrics = _build_screen_metrics()
                    screen_info = screen_metrics["screen"]
                    ...
                    page = browser.new_page(
                        ...
                        device_scale_factor=screen_metrics["devicePixelRatio"],
                    )

                    # Apply stealth patches
                    Stealth().apply_stealth_sync(page)  # type: ignore[union-attr]
                    ...
                    # Human-like delay before finishing
                    _human_delay(50, 200)
                    ...
                    if elapsed_ms < rate_limit_ms:
                        deficit = rate_limit_ms - elapsed_ms
                        time.sleep(deficit / 1000)
                        elapsed_ms = int(rate_limit_ms)
