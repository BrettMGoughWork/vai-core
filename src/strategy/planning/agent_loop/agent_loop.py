...
from src.strategy.planning.segments.execution import (
...
from src.strategy.planning.segments.trace import (
    SegmentTrace,
    build_segment_trace,
...
from src.strategy.planning.subgoals.reflection import (
    SubgoalReflectionResult,
    evaluate_subgoal_completion,
    evaluate_subgoal_progress,
    reflect_on_subgoal,
)
from src.strategy.planning.subgoals.trace import (
    SubgoalTrace,
    build_subgoal_trace,
...
def run_agent_loop(
    ...
    for cycle in range(max_cycles):
        ...
        subgoal_trace: SubgoalTrace | None = None
        ...
        if all_segments_done:
            ...
            subgoal_trace = sg_result.get("subgoal_trace")
        ...
        # ── 12b. Drift pipeline (2.14) ─────────────────────────────────────
        # Wired behind a feature flag for incremental rollout.
        if DRIFT_PIPELINE_ENABLED:
            prev_conf, prev_budget = extract_pipeline_state(memory)
            pipeline_result = run_drift_pipeline(
                segment_drift=segment_trace.drift if segment_trace else [],
                subgoal_drift=subgoal_trace.drift if subgoal_trace else [],
                prev_confirmation=prev_conf,
                prev_budget=prev_budget,
            )
            store_pipeline_state(memory, pipeline_result)
            # Augment drift_result with pipeline metadata so the error
            # evaluator can use it.
            if drift_result is not None:
                drift_result["drift_pipeline"] = pipeline_result
