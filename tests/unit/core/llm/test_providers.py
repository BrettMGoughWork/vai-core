...
from src.core.llm.providers.anthropic import AnthropicClient, _to_claude_messages
from src.core.llm.providers.openai import OpenAIClient
from src.core.llm.llm_factory import create as factory_create
...
class TestAnthropicClientChat:
    ...
    def _call_with_mock(self, client, body, **kwargs):
        ...
        with patch("src.core.llm.providers.anthropic.request.urlopen", side_effect=fake_urlopen):
    ...
    def test_http_error_raises_runtime_error(self, client):
        ...
        with patch("src.core.llm.providers.anthropic.request.urlopen", side_effect=http_err):
    ...
    def test_url_error_raises_runtime_error(self, client):
        ...
        with patch("src.core.llm.providers.anthropic.request.urlopen", side_effect=url_err):
            with pytest.raises(RuntimeError, match="Anthropic"):
                client.chat(model="m", messages=[{"role": "user", "content": "hi"}])

    def test_missing_api_key_raises_value_error(self):
        with patch.dict(os.environ, {}, clear=True):
            with patch("src.core.llm.providers.anthropic.load_dotenv"):
...
class TestOpenAIClientChat:
    ...
    def _call_with_mock(self, client, body, **kwargs):
        ...
        with patch("src.core.llm.providers.openai.request.urlopen", side_effect=fake_urlopen):
            result = client.chat(model=model, messages=messages, **kwargs)
        return result, captured.get("req")
    ...
    def test_http_error_raises_runtime_error(self, client):
        ...
        with patch("src.core.llm.providers.openai.request.urlopen", side_effect=http_err):
            with pytest.raises(RuntimeError, match="429"):
                client.chat(model="m", messages=[{"role": "user", "content": "hi"}])

    def test_missing_api_key_raises_value_error(self):
        with patch.dict(os.environ, {}, clear=True):
            with patch("src.core.llm.providers.openai.load_dotenv"):
                with pytest.raises(ValueError, match="API key"):
...
class TestLLMFactory:
    def test_known_provider_returns_client_instance(self):
        ...
        with patch("src.core.llm.llm_factory.PROVIDER_CLIENTS", {"fake": FakeClient}):
    ...
    def test_provider_name_is_case_insensitive(self):
        ...
        with patch("src.core.llm.llm_factory.PROVIDER_CLIENTS", {"fakeprovider": FakeClient}):
    ...
    def test_extra_kwargs_not_accepted_by_client_are_filtered(self):
        ...
        with patch("src.core.llm.llm_factory.PROVIDER_CLIENTS", {"strict": StrictClient}):
    ...
    def test_model_passed_when_accepted_by_client(self):
        ...
        with patch("src.core.llm.llm_factory.PROVIDER_CLIENTS", {"maware": ModelAwareClient}):
    ...
    def test_accepted_kwargs_forwarded_to_client(self):
        ...
        with patch("src.core.llm.llm_factory.PROVIDER_CLIENTS", {"conf": ConfigurableClient}):
