"""
Tests for Phase 2.13.1 — Full Agent Loop V3
(``src.core.planning.agent_loop.agent_loop_v3``).
"""
...
from src.core.planning.segments.trace import SegmentTrace
from src.core.planning.subgoals.execution import (
    SubgoalExecutionPhase,
    SubgoalExecutionState,
)
from src.core.planning.subgoals.trace import SubgoalTrace
...
class TestAgentTrace:
    """Tests for the AgentTrace frozen dataclass."""

    def test_construction_empty(self):
        """AgentTrace constructs with empty lists."""
        t = AgentTrace(cycles=[], subgoals=[], segments=[])
        assert t.cycles == []
        assert t.subgoals == []
        assert t.segments == []
    ...
    def test_to_dict_json_safe(self):
        ...
        t = AgentTrace(cycles=[{"c": 1}], subgoals=[{"s": 1}], segments=[{"seg": 1}])
        d = t.to_dict()
        assert _is_json_safe(d)
    ...
    def test_hash_deterministic(self):
        ...
        t1 = AgentTrace(cycles=[], subgoals=[], segments=[])
        t2 = AgentTrace(cycles=[], subgoals=[], segments=[])
...
class TestAgentLoopResultErrorField:
    ...
    def test_error_included_in_trace_when_present(self):
        """AgentTrace.errors is populated when errors occur."""
