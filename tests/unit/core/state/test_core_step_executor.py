"""
Tests for src.core.state.core_step_executor — CoreStepExecutor, classify_step, isdone.
...
def no_skills():
    ...
    with patch("src.core.state.core_step_executor.SkillRegistry.all_specs_for_agent", return_value=[]):
        yield
...
class TestCoreStepExecutorToolPath:
    def _patched_tool_run(self, tool_result):
        ...
        return (
            patch("src.core.state.core_step_executor.SkillRegistry.all_specs_for_agent", return_value=[]),
            patch("src.core.state.core_step_executor.select_tool", return_value=FakeTool("echo", tool_result)),
            patch("src.core.state.core_step_executor.execute_with_retry", return_value=tool_result),
        )
...
class TestCircuitBreaker:
    def test_open_circuit_blocks_tool_and_returns_safe_failure(self):
        ...
        with patch("src.core.state.core_step_executor.SkillRegistry.all_specs_for_agent", return_value=[]), \
             patch("src.core.state.core_step_executor.select_tool", return_value=tool):
